...
import { suspendSubscription } from "@/lib/admin/subscription-actions";
...
export async function POST(req: Request, { params }: Params) {
  const auth = await requireAdminApi("subscription.suspend", req);
  ...
  const updated = await executeAdminAction({
    ...
    request: req,
    exec: async (tx) => {
      const result = await suspendSubscription(tx, params.id);
